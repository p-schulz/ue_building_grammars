...
bool FBuildingFootprintAssembler::ComputeFootprintBoundsCenter(const TArray<FBuildingFootprint>& Footprints, double& OutCenterLatitude, double& OutCenterLongitude)
{
	...
	bool bFoundAnyPoint = false;

	auto AccumulateRing = [&MinLat, &MaxLat, &MinLon, &MaxLon, &bFoundAnyPoint](const TArray<FVector2D>& Points)
	{
		for (const FVector2D& Point : Points)
		{
			// FBuildingFootprint stores rings as (X=Lon, Y=Lat) -- see BuildingFootprintAssembler.cpp's
			// ResolveRing.
			MinLon = FMath::Min(MinLon, Point.X);
			MaxLon = FMath::Max(MaxLon, Point.X);
			MinLat = FMath::Min(MinLat, Point.Y);
			MaxLat = FMath::Max(MaxLat, Point.Y);
			bFoundAnyPoint = true;
		}
	};
	...
	{
		AccumulateRing(Footprint.OuterRing);
		for (const FGrammarRing& Hole : Footprint.Holes)
		{
			AccumulateRing(Hole.Points);
		}
	}
