...
namespace GrammarRoofDirection
{
	FVector2D DirectionFromCardinalOrAngle(const FString& Value)
	{
		...
		// X=North, Y=East here -- matches FLocalTangentPlaneProjection's documented axis convention
		// (see that class's header comment), which every footprint/placement coordinate in this
		// pipeline is expressed in. Do not swap this back to the more "conventional" X=East, Y=North
		// without also flipping the projection -- an earlier version of this table assumed X=East,
		// Y=North, which silently rotated every roof:orientation/roof:direction/ridge_direction-
		// tagged building's ridge 90 degrees from what the OSM tag actually specified (buildings
		// without such tags were unaffected -- they fall back to the footprint's longest axis, which
		...
		return FGrammarGeometry2D::Normalize2D(FVector2D(FMath::Cos(AngleRadians), FMath::Sin(AngleRadians)));
	}
